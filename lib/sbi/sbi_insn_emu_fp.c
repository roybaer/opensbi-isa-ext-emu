...
int sbi_insn_emu_load_fp(ulong insn, struct sbi_trap_context *tcntx)
{
	struct sbi_trap_regs *regs = &tcntx->regs;

	/* If insn is FLH, simply use the misaligned load handler */
...
int sbi_insn_emu_store_fp(ulong insn, struct sbi_trap_context *tcntx)
{
	struct sbi_trap_regs *regs = &tcntx->regs;

	/* If insn is FSH, simply use the misaligned store handler */
	if ((regs->mstatus & MSTATUS_FS) != 0 &&
...
static u32 convert_f16_to_f32(u16 val)
{
	/* special case: +/- zero */
	if ((val & 0x7fff) == 0)
		return (u32)val << 16;
...
static u64 convert_f16_to_f64(u16 val)
{
	/* special case: +/- zero */
	if ((val & 0x7fff) == 0)
		return (u64)val << 48;
...
static u16 convert_f32_to_f16(u32 val, u32 *fcsr, int rm)
{
	...
	static const u16 rm_bias[2][2][5] = {
		{ { 0x1000, 0x0000, 0x0000, 0x1fff, 0x1fff },
		  { 0x0fff, 0x0000, 0x0000, 0x1fff, 0x1fff } },
		{ { 0x1000, 0x0000, 0x1fff, 0x0000, 0x1fff },
		  { 0x0fff, 0x0000, 0x1fff, 0x0000, 0x1fff } }
	};
	...
	static const u32 inf_threshold[2][5] = {
		{ 0x477ff000, 0x47800000, 0x47800000, 0x477fe000, 0x477fe000 },
		{ 0x477ff000, 0x47800000, 0x477fe000, 0x47800000, 0x477fe000 }
	};
	...
	static const u32 subnorm_threshold[2][5] = {
		/* TODO */
		{ 0x387ff000, 0x387fffff, 0x387fffff, 0x387fe000, 0x387fe000 },
		{ 0x387ff000, 0x387fffff, 0x387fe000, 0x387fffff, 0x387fe000 }
	};
	...
	if ((val & 0x7fffffff) < subnorm_threshold[sign][rm]) {
		...
		unsigned shiftval = 113 - ((val >> 23) & 0xff);
		if (val & (0x007fffff >> (10 - shiftval)))
			*fcsr |= FFLAG_INEXACT;
		return (sign << 15) |
		       ((((val & 0x007fffff) | 0x00800000) +
			 (rm_bias[sign][(val >> (13 + shiftval)) & 1][rm] >>
			  shiftval)) >>
			(13 + shiftval));
	}
	/* no special case */
...
static u16 convert_f64_to_f16(u64 val, u32 *fcsr, int rm)
{
	...
	static const u64 rm_bias[2][2][5] = {
		{ { 0x20000000000, 0, 0, 0x3ffffffffff, 0x3ffffffffff },
		  { 0x1ffffffffff, 0, 0, 0x3ffffffffff, 0x3ffffffffff } },
		{ { 0x20000000000, 0, 0x3ffffffffff, 0, 0x3ffffffffff },
		  { 0x1ffffffffff, 0, 0x3ffffffffff, 0, 0x3ffffffffff } }
	};
	...
	static const u64 inf_threshold[2][5] = {
		{ 0x40effe0000000000, 0x40f0000000000000, 0x40f0000000000000,
		  0x40effc0000000000, 0x40effc0000000000 },
		{ 0x40effe0000000000, 0x40f0000000000000, 0x40effc0000000000,
		  0x40f0000000000000, 0x40effc0000000000 }
	};
	...
	static const u64 subnorm_threshold[2][5] = {
		{ 0x3f0ffe0000000000, 0x3f0fffffffffffff, 0x3f0fffffffffffff,
		  0x3f0ffc0000000000, 0x3f0ffc0000000000 },
		{ 0x3f0ffe0000000000, 0x3f0fffffffffffff, 0x3f0ffc0000000000,
		  0x3f0fffffffffffff, 0x3f0ffc0000000000 }
	};
	...
	if ((val & 0x7ff0000000000000) < 0x3e30000000000000) {
		*fcsr |= FFLAG_UNDERFLOW | FFLAG_INEXACT;
		return (val & 0x8000000000000000) >> 48;
	}
	/* replace too big numbers with +/- infinity */
	...
	if ((val & 0x7fffffffffffffff) < subnorm_threshold[sign][rm]) {
		...
		if (val & (0x000fffffffffffff >> (10 - shiftval)))
			*fcsr |= FFLAG_INEXACT;
		return (sign << 15) |
		       ((((val & 0x000fffffffffffff) | 0x0010000000000000) +
			 (rm_bias[sign][(val >> (42 + shiftval)) & 1][rm] >>
			  shiftval)) >>
			(42 + shiftval));
	}
	/* no special case */
...
static u32 round_f32(u32 val, u32 *fcsr, int rm, bool set_nx)
{
	...
	static const u32 rm_bias[2][2][5] = {
		{ { 0x400000, 0x000000, 0x000000, 0x7fffff, 0x7fffff },
		  { 0x3fffff, 0x000000, 0x000000, 0x7fffff, 0x7fffff } },
		{ { 0x400000, 0x000000, 0x7fffff, 0x000000, 0x7fffff },
		  { 0x3fffff, 0x000000, 0x7fffff, 0x000000, 0x7fffff } }
	};

	/* rounded small non-zero value.
	 * sign, rounding mode */
	static const u32 small_nz[2][5] = {
		{ 0x00000000, 0x00000000, 0x00000000, 0x3f800000, 0x3f800000 },
		{ 0x80000000, 0x80000000, 0xbf800000, 0x80000000, 0xbf800000 }
	};
	...
	if ((val & 0x7f800000) >= 0x4b000000)
		return val;
	/* handle values that can only yield 0 or 1 */ /* FIXME: "<", instead? */
	if ((val & 0x7f800000) <= 0x34000000) {
		if (set_nx)
			*fcsr |= FFLAG_INEXACT;
		if (rm == RM_FIELD_RNE && (val & 0x7f800000) == 0x34000000)
			return (val & 0x80000000) | 0x3f800000;
		return small_nz[val >> 31][rm];
	}
...
static u64 round_f64(u64 val, u32 *fcsr, int rm, bool set_nx)
{
	 ...
	 * sign, future LSB, rounding mode */
	static const u64 rm_bias[2][2][5] = {
		{ { 0x8000000000000, 0, 0, 0xfffffffffffff, 0xfffffffffffff },
		  { 0x7ffffffffffff, 0, 0, 0xfffffffffffff, 0xfffffffffffff } },
		{ { 0x8000000000000, 0, 0xfffffffffffff, 0, 0xfffffffffffff },
		  { 0x7ffffffffffff, 0, 0xfffffffffffff, 0, 0xfffffffffffff } }
	};

	/* rounded small non-zero value.
	 * sign, rounding mode */
	static const u64 small_nz[2][5] = {
		{ 0x0000000000000000, 0x0000000000000000, 0x0000000000000000,
		  0x3ff0000000000000, 0x3ff0000000000000 },
		{ 0x8000000000000000, 0x8000000000000000, 0xbff0000000000000,
		  0x8000000000000000, 0xbff0000000000000 }
	};
	...
	if ((val & 0x7ff0000000000000) >= 0x4330000000000000)
		return val;
	/* handle values that can only yield 0 or 1 */ /* FIXME: "<", instead? */
	if ((val & 0x7ff0000000000000) <= 0x3cb0000000000000) {
		if (set_nx)
			*fcsr |= FFLAG_INEXACT;
		if (rm == RM_FIELD_RNE &&
		    (val & 0x7ff0000000000000) == 0x3cb0000000000000)
			return (val & 0x8000000000000000) | 0x3ff0000000000000;
		return small_nz[val >> 63][rm];
	}
	/* handle all other values */
...
static u16 round_f16(u16 val, u32 *fcsr, int rm, bool set_nx)
{
	 ...
	 * sign, future LSB, rounding mode */
	static const u16 rm_bias[2][2][5] = {
		{ { 0x200, 0x000, 0x000, 0x3ff, 0x3ff },
		  { 0x1ff, 0x000, 0x000, 0x3ff, 0x3ff } },
		{ { 0x200, 0x000, 0x3ff, 0x000, 0x3ff },
		  { 0x1ff, 0x000, 0x3ff, 0x000, 0x3ff } }
	};

	/* rounded small non-zero value.
	 * sign, rounding mode */
	static const u16 small_nz[2][5] = {
		{ 0x0000, 0x0000, 0x0000, 0x3c00, 0x3c00 },
		{ 0x8000, 0x8000, 0xbc00, 0x8000, 0xbc00 }
	};
	...
	if ((val & 0x7c00) >= 0x6400)
		return val;
	/* handle values that can only yield 0 or 1 */ /* FIXME: "<", instead? */
	if ((val & 0x7c00) <= 0x1400) {
		if (set_nx)
			*fcsr |= FFLAG_INEXACT;
		if (rm == RM_FIELD_RNE && (val & 0x7c00) == 0x1400)
			return (val & 0x8000) | 0x3c00;
		return small_nz[val >> 15][rm];
	}
	/* handle all other values */
...
int sbi_insn_emu_op_fp(ulong insn, struct sbi_trap_context *tcntx)
{
	struct sbi_trap_regs *regs = &tcntx->regs;
	u64 val;
	u32 fcsr;
	...
	default:
		...
		case INSN_MATCH_FMINM_H: {
			u16 rs1 = GET_F16_RS1(insn, regs);
			u16 rs2 = GET_F16_RS2(insn, regs);
		...
		case INSN_MATCH_FMINM_S: {
			u32 rs1 = GET_F32_RS1(insn, regs);
			u32 rs2 = GET_F32_RS2(insn, regs);
		...
		case INSN_MATCH_FMINM_D: {
			u64 rs1 = GET_F64_RS1(insn, regs);
			u64 rs2 = GET_F64_RS2(insn, regs);
